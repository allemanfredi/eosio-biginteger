...
void BigUnsigned::initFromSignedPrimitive(X x) {
	...
	if (x < 0)
		...
		initFromPrimitive(x);
		*/
	eosio::check( x >= 0, "BigUnsigned constructor: Cannot construct a BigUnsigned from a negative number");
...
X BigUnsigned::convertToPrimitive() const {
	...
	eosio::check( 0, "BigUnsigned::to<Primitive>: Value is too big to fit in the requested type");
	return 0;
}
...
X BigUnsigned::convertToSignedPrimitive() const {
	...
	*/
	...
	eosio::check( x < 0, "BigUnsigned::to(Primitive): Value is too big to fit in the requested type");
